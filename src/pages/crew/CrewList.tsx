...
export const CrewList: React.FC = () => {
    ...
    return (
        <>
            {
                crews?.map((crew) => (<CrewEntry marginColor={crew.hex_color} key={crew.uuid}>
                    <h2>{crew.name}</h2>
                    <p>{crew.description}</p>
                    {!crew.is_applyable?(<p><b>Kan ikke søkes til - du må ofte ha erfaring med LANet for å bli invitert hit</b></p>):null}
                </CrewEntry>))
            }
        </>
    )
